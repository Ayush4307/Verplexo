  ...
  return (
    <div className="bg-white min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="py-12">
          <h1 className="text-4xl font-extrabold text-zinc-900 tracking-tight">Recent Work</h1>
          <p className="mt-4 text-xl text-zinc-500">Explore some of the scalable solutions we've delivered.</p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pb-24">
          {projects.map((project, idx) => (
            <div key={idx} className="group relative overflow-hidden rounded-2xl bg-zinc-50 border border-zinc-200 p-8 hover:shadow-lg transition-all">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <span className="text-xs font-semibold uppercase tracking-wider text-brand mb-2 block">{project.type}</span>
                  <h3 className="text-2xl font-bold text-zinc-900 group-hover:text-brand transition-colors">{project.name}</h3>
                </div>
                <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-sm text-zinc-400 group-hover:text-brand group-hover:bg-brand/10 transition-colors">
                  <ExternalLink size={20} />
                </div>
              </div>
              <p className="text-zinc-600 mb-8">{project.desc}</p>
              <div className="h-48 bg-zinc-200 rounded-lg w-full mt-auto relative overflow-hidden">
