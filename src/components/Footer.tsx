...
import { SITE_NAME, EMAIL, PHONE, LOCATION, SOCIAL_LINKS, TAGLINE } from '../constants/brand'
...
const footerLinks = {
  Services: [
    ...
    { label: 'Custom Platforms', path: '/services' },
  ],
  Company: [
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 mb-12">
          ...
          <div className="lg:col-span-2">
            <Link to="/" className="flex items-center gap-2 group mb-4">
              ...
              <span className="font-bold text-xl tracking-tight text-zinc-900 dark:text-zinc-100">{SITE_NAME}</span>
            </Link>
            <p className="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed mb-6 max-w-xs">
              {TAGLINE}. We build scalable products for ambitious teams.
            </p>

            {/* Contact info */}
            <ul className="space-y-2 mb-6">
              {contactDetails.map(({ Icon, label, href }) => (
                <li key={label} className="flex items-center gap-2.5 text-sm text-zinc-500 dark:text-zinc-400">
                  <Icon size={14} className="text-brand shrink-0" />
                  {href ? (
                    <a href={href} className="hover:text-brand transition-colors">{label}</a>
                  ) : (
                    <span>{label}</span>
                  )}
                </li>
            ...
            <div className="flex gap-2">
              {socials.map(({ Icon, label, href }) => (
                <a
                  ...
                  aria-label={label}
                  className="w-9 h-9 rounded-lg border border-zinc-200 dark:border-zinc-700 flex items-center justify-center text-zinc-500 dark:text-zinc-400 hover:text-brand hover:border-brand dark:hover:border-brand transition-colors"
                >
                  <Icon />
                </a>
              ))}
          ...
          {Object.entries(footerLinks).map(([section, links]) => (
            <div key={section}>
              <h3 className="text-xs font-bold text-zinc-900 dark:text-zinc-100 tracking-widest uppercase mb-4">
                {section}
              </h3>
              <ul className="space-y-2.5">
                {links.map((link) => (
                  <li key={link.label}>
                    <Link
                      to={link.path}
                      className="text-sm text-zinc-500 dark:text-zinc-400 hover:text-brand dark:hover:text-brand transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
